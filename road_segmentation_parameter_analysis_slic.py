...
for img_name in img_list:   
    ...
    alfa = -3  # coefficient of the weighted area term A(phi)
    # n_segments
    n_segments_test_array=np.zeros((50,4))
    for idx,val in enumerate(np.linspace(start = 100, stop = 5000, num = 50)):
        n_segments=val
        _Completeness_ours, _Correctness_ours, _Quality_ours=ours(H,W,mu,lmda,alfa,n_segments,parameter_analysis_dir,road_segmentaion_reference_img_dir,'slic_K='+str(n_segments))
        n_segments_test_array[idx]=np.array([val,_Completeness_ours, _Correctness_ours, _Quality_ours])
        

    np.save('slic_K.npy',n_segments_test_array)